...
document.querySelectorAll('.faq_wrap').forEach(faqWrap => {
    ...
    const showLeft = () => {
        ...
        faqRight.style.transform = 'translateX(160px)';
    };
    const hideLeft = () => {
...
document.querySelectorAll('.faq_spBox').forEach(faqBox => {
    const q = faqBox.querySelector('.faq_sp-q');
    const a = faqBox.querySelector('.faq_sp-a');

    q.addEventListener('click', () => {
        const isActive = q.classList.contains('active');

        // すべてのFAQを閉じる
        document.querySelectorAll('.faq_sp-q').forEach(el => el.classList.remove('active'));
        document.querySelectorAll('.faq_sp-a').forEach(el => el.classList.remove('active'));
